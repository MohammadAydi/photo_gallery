...
public class YuvColorSpace : BaseColorSpace
{
    ...
    protected override void BuildPointCloud()
    {
        ...
        for (var b = 0; b <= 255; b += step)
        {
            ...
            var (Y, U, V) = ColorMath.RgbToYuv(rn, gn, bn);

            // Y: 0-1 → -1..1, U: -0.436..0.436, V: -0.615..0.615
            var x = U / 0.436f;
            var y = Y * 2f - 1f;
            var z = V / 0.615f;
    ...
    protected override void BuildOutline()
    {
        ...
        var axes = AxisBuilder.BuildCartesianAxes(
            (1f, 0f, 0f), (0.2f, 0.2f, 1f), // U → blue
            (0f, 1f, 0f), (0.8f, 0.8f, 0.8f), // Y → gray
            (0f, 0f, 1f), (1f, 0.2f, 0.2f), // V → red
            0f, 0f, 0f);

        float[] combined = [..v, ..axes];
        (VaoOutline, VboOutline) = UploadStatic(v.ToArray());
        OutlineVertCount = v.Count / 6;
    }
    ...
    {
        return (ch[1] / 0.436f, ch[0] * 2f - 1f, ch[2] / 0.615f);
        // ch = [Y, U, V]
    }
}